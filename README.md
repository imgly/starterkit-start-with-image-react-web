...
```bash
git clone https://github.com/imgly/starterkit-start-with-image-ts-web.git
cd starterkit-start-with-image-ts-web
```
...
```
starterkit-start-with-image-ts-web/
├── src/
│   ├── index.ts              # Application entry with image selection sidebar
│   └── imgly/
│       ├── index.ts          # Editor initialization with createFromImage
│       ├── config/           # Design editor configuration
│       └── plugins/
│           └── background-removal.ts
├── index.html                # Layout with sidebar and editor container
├── package.json
└── vite.config.ts
```
